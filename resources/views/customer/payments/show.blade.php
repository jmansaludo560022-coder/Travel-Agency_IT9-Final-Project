...
<div class="max-w-2xl mx-auto space-y-4">
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
        ...
        <div class="p-6 grid grid-cols-2 gap-4 text-sm">
            ...
            <div><p class="text-xs text-gray-500">Payment Method</p><p class="text-gray-700 dark:text-gray-300 mt-0.5 capitalize">{{ str_replace('_', ' ', $payment->payment_method) }}</p></div>
            <div><p class="text-xs text-gray-500">Payment Date</p><p class="text-gray-700 dark:text-gray-300 mt-0.5">{{ $payment->payment_date->format('F d, Y') }}</p></div>
            <div><p class="text-xs text-gray-500">Reference No.</p><p class="font-mono text-gray-900 dark:text-white mt-0.5">{{ $payment->payment_ref_no }}</p></div>
            <div>
                ...
                @endphp
                <span class="mt-1 inline-block px-2.5 py-0.5 text-xs font-medium rounded-full {{ $ps }}">
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr>
                    <td class="px-6 py-3 text-gray-700 dark:text-gray-300">{{ \Carbon\Carbon::parse($schedule->due_date)->format('M d, Y') }}</td>
                    <td class="px-6 py-3 font-semibold text-gray-900 dark:text-white">${{ number_format($schedule->amount_due, 2) }}</td>
                    <td class="px-6 py-3">
                        @php
                            $ss = [
                        ...
                        @endphp
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $ss }}">{{ ucfirst($schedule->status) }}</span>
                    </td>
                </tr>
                @endforeach
            </tbody>
