...
<div class="flex items-center gap-1 mb-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-1 w-fit">
    @foreach(['all'=>'All','pending'=>'Pending','confirmed'=>'Confirmed','cancelled'=>'Cancelled','completed'=>'Completed','fully_paid'=>'Fully Paid'] as $val=>$lbl)
        <a href="{{ route('admin.bookings.index', ['status' => $val === 'all' ? null : $val]) }}"
            class="px-3 py-1.5 text-xs font-medium rounded-lg transition
                {{ (request('status', 'all') === $val) ? 'bg-sky-500 text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800' }}">
            {{ $lbl }}
        </a>
    @endforeach
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if($booking->booking_status === 'pending')
                                <form action="{{ route('admin.bookings.update-status', $booking->id) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit"
                                        class="px-2.5 py-1 text-xs font-semibold bg-green-500 hover:bg-green-600 text-white rounded-lg transition"
                                ...
                                <form action="{{ route('admin.bookings.update-status', $booking->id) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit"
                                        class="px-2.5 py-1 text-xs font-semibold bg-red-500 hover:bg-red-600 text-white rounded-lg transition"
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-6 py-10 text-center text-gray-400">No bookings found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
