    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-0.5">Add a payment on behalf of a customer</p>
            </div>
        ...
        <form action="{{ route('admin.payments.store') }}" method="POST" id="paymentForm" class="p-6 space-y-5">
            @csrf
            @php
                ...
                <select name="booking_id" id="booking_id" required onchange="onBookingChange()" class="{{ $input }}">
                    ...
                    @foreach($bookings as $b)
                        ...
                        <option value="{{ $b->id }}"
                            ...
                            data-name="{{ $b->travelPackage->package_name }}"
                            data-customer="{{ $b->customer->cus_fn }} {{ $b->customer->cus_ln }}"
            ...
            {{-- Booking summary (shown after selection) --}}
...
function updateAmountHint() {
    ...
    } else if (val === balance || Math.abs(val - balance) < 0.001) {
...
document.addEventListener('DOMContentLoaded', function () {
    toggleSchedules();
    // If a booking was pre-selected (from query string or old input), trigger the change
    const sel = document.getElementById('booking_id');
