...
<div class="bg-white rounded-lg shadow-sm">
    <div class="p-6 border-b">
        <h2 class="text-xl font-semibold text-gray-800">Create New Travel Package</h2>
    </div>

    <form action="{{ route('agent.packages.store') }}" method="POST" class="p-6">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                <label for="package_name" class="block text-sm font-medium text-gray-700 mb-2">Package Name</label>
                <input type="text" name="package_name" id="package_name" value="{{ old('package_name') }}" required
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <select name="package_type" id="package_type"
                    ...
                    @foreach(['Beach', 'Adventure', 'Cultural', 'Nature', 'City', 'Luxury', 'Family', 'Honeymoon', 'Wildlife', 'Cruise'] as $type)
                        <option value="{{ $type }}" {{ old('package_type') === $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
                @error('package_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div>
                ...
                <input type="number" name="package_cost" id="package_cost" step="0.01" min="0" value="{{ old('package_cost') }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @error('package_cost')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="slots_available" id="slots_available" min="0" value="{{ old('slots_available', 0) }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500">
        ...
        <div class="mb-6">
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-sky-500">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
